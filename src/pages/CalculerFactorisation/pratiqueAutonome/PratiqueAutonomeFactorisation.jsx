...
export default function PratiqueAutonomeFactorisation() {
  const [exercice, setExercice] = useState(1);
  const [score, setScore] = useState(0);
  const [niveau, setNiveau] = useState(0);
  const [currentCalcul, setCurrentCalcul] = useState(genererCalculFactorisation());
  ...
  function genererCalculFactorisation() {
    let K = Math.floor(Math.random() * 8) + 2;
    let baseRonde = 10;
    let isAddition = Math.random() > 0.5;

    let B = Math.floor(Math.random() * 4) + 1;
    let A = isAddition ? baseRonde - B : baseRonde + B;
    let signe = isAddition ? '+' : '-';

    let resultatAttendu = K * baseRonde;

    return {
      affichage: `${A} * ${K} ${signe} ${B} * ${K}`,
  ...
  async function discuterErreur(feedbackErreur, isUserMessage = false) {
    ...
    const A = currentCalcul.A;
    const B = currentCalcul.B;
  ...
  function verifierReponse() {
    const e1 = etape1.replace(/\s+/g, '');
    const e2 = etape2.replace(/\s+/g, '');
    const e3 = Number(etape3);

    const expE1_A = `${currentCalcul.K}*(${currentCalcul.A}${currentCalcul.signe}${currentCalcul.B})`;
    const expE1_B = `(${currentCalcul.A}${currentCalcul.signe}${currentCalcul.B})*${currentCalcul.K}`;
    const isE1Correct = (e1 === expE1_A || e1 === expE1_B);

    const expE2_A = `${currentCalcul.K}*${currentCalcul.baseRonde}`;
    const expE2_B = `${currentCalcul.baseRonde}*${currentCalcul.K}`;
  ...
  function exerciceSuivant() {
    if (exercice < totalQuestions) {
      ...
      setCurrentCalcul(genererCalculFactorisation());
      setEtape1("");
      setEtape2("");
      setEtape3("");
      setMessage("");
            ...
            <>
              ...
              <div className="d-flex flex-column gap-3 mx-auto mb-5" style={{ maxWidth: '400px' }}>
                <div className="input-group shadow-sm rounded-3 overflow-hidden border border-light">
                  <span className="input-group-text bg-light border-0 fw-bold text-secondary" style={{ width: '100px' }}>Étape 1</span>
                  <input type="text" className="form-control text-center border-0 py-2 custom-input-wrapper" placeholder={`Ex: 5 * (13 - 3)`} value={etape1} onChange={(e) => setEtape1(e.target.value)} disabled={isCorrect === true} />
                </div>

                <div className="input-group shadow-sm rounded-3 overflow-hidden border border-light">
                  <span className="input-group-text bg-light border-0 fw-bold text-secondary" style={{ width: '100px' }}>Étape 2</span>
                  <input type="text" className="form-control text-center border-0 py-2 custom-input-wrapper" placeholder={`Ex: 5 * 10`} value={etape2} onChange={(e) => setEtape2(e.target.value)} disabled={isCorrect === true} />
                </div>

                <div className="input-group shadow-sm rounded-3 overflow-hidden border border-light">
                  <span className="input-group-text bg-light border-0 fw-bold text-secondary" style={{ width: '100px' }}>Étape 3</span>
            ...
            <div className="animate__animated animate__fadeIn py-4">
              ...
              {(score / totalQuestions) >= 0.75 ? (
                <div className="bg-success bg-opacity-10 border border-success border-opacity-25 rounded-4 p-4 mt-4">
                  ...
                  <button className="btn btn-success rounded-pill px-5 py-3 fw-bold shadow-sm btn-hover-scale w-100" onClick={() => {
                ...
                <div className="bg-danger bg-opacity-10 border border-danger border-opacity-25 rounded-4 p-4 mt-4">
                  ...
                  <button className="btn btn-danger rounded-pill px-5 py-3 fw-bold shadow-sm btn-hover-scale w-100" onClick={() => navigate("/ModelageCalculFactorisation")}>
                    Retour à la leçon 📖
                  </button>
                </div>
              )}
