...
export default function PratiqueAutonomeTesterEgalite2() {
  ...
  function genererEquationIdentique() {
    ...
    while (c === a) { c = (Math.floor(Math.random() * 5) + 1) * 0.5; }
    ...
    setEquation({
      ...
      resGaucheAttendu: Number((a * xValeur + b).toFixed(1)),
      resDroiteAttendu: Number((c * xValeur + d).toFixed(1))
    });
  ...
  function verifierReponse() {
    ...
    if (progression === 0) {
      ...
      if (parseFloat(calculGauche.toString().replace(',', '.')) === equation.resGaucheAttendu) { setIsCorrect(null); setProgression(3); setMessage(""); }
      else { setIsCorrect(false); setMessage("Le calcul de gauche est faux."); discuterErreur(`Je trouve ${calculGauche}`); }
    } else if (progression === 3) {
      if (parseFloat(calculDroite.toString().replace(',', '.')) === equation.resDroiteAttendu) {
  ...
  return (
    <LayoutPratiqueIA
      ...
      onNextLevel={() => navigate("/PratiqueGuideTesterEgalite3")}
      onRetry={() => navigate("/ModelageTesterEgalite2")}
            ...
            {progression >= 0 && (
              <div className="mt-3 text-start">
                ...
                {progression > 0 ? <div className="fw-bolder fs-5 text-success text-center bg-light rounded-3 py-2">{inputGauche}</div> : (
                  <input type="text" className="form-control text-center custom-input-wrapper text-primary fw-bold" value={inputGauche} onChange={e => setInputGauche(e.target.value)} onKeyDown={e => e.key === 'Enter' && verifierReponse()} disabled={isCorrect === false} />
                )}
              </div>
            )}
