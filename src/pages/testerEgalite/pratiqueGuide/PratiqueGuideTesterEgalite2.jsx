...
function PratiqueGuideTesterEgalite2() {
  ...
  useEffect(() => {
    ...
    if (progression === 0) {
      ajouterMessageIA(`Commençons ! Remplace la lettre x par ${equation.x} dans le membre de gauche (${equation.gaucheTexte}). N'oublie pas le signe * !`);
      ...
      if (equation.resGaucheAttendu === equation.resDroiteAttendu) {
        ajouterMessageIA(`Parfait ! On trouve ${g} des deux côtés. L'égalité est donc VRAIE pour x = ${equation.x}.`);
      } else {
        ajouterMessageIA(`Parfait ! On trouve ${g} à gauche et ${d} à droite. Les résultats sont différents, l'égalité est donc FAUSSE pour x = ${equation.x}.`);
      }
    }
            ...
            <div className="alert alert-success text-center rounded-4 shadow-sm p-4 animate__animated animate__fadeIn">
              ...
              <p className="small mb-3 text-muted">
                {equation.resGaucheAttendu === equation.resDroiteAttendu
                  ? "Les deux résultats sont identiques : l'égalité est VRAIE pour cette valeur."
                  : "Les deux résultats sont différents : l'égalité est FAUSSE pour cette valeur."
                }
              </p>
              <button className="btn btn-success rounded-pill px-5 fw-bold" onClick={() => navigate("/PratiqueAutonomeTesterEgalite2")}>
...
export default PratiqueGuideTesterEgalite2;